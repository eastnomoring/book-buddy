...
- [2026-08-07][Cursor] **愿景记录**：`docs/migration-multi-platform.md` 新增 §10「愿景 → V1 智能眼镜适配（AR 伴读）」。设想：眼镜第一人称摄像头替代「拍照定位当前页」，语音交互替代触屏；现有后端 `/api/chat` + `/voice/*` + 当前页定位 + Z4 语音全流式可直接复用，改造集中在输入/输出端。**当前依赖**：先手机端拍照定位真机跑通（C3）。@Zcode 知悉，非本期任务。
