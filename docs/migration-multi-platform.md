...
> 远期方向，非本期任务，仅记录意图、供后续排期参考。

### V1 — 智能眼镜适配（AR 伴读）

**设想**：把「拍照定位当前页」换成眼镜的第一人称摄像头，语音交互替代触屏。用户戴着眼镜翻书，眼镜看到哪页就回答哪页的问题，听到讲解，不再看手机/电脑屏幕。

**为什么契合**：现有架构几乎可以直接复用——

- 「当前页定位」（拍书页 → 找书内上下文）= 眼镜摄像头的天然输入
- 语音链路（ASR → LLM → TTS 全流式，Z4 已完成）= 眼镜主要交互
- FastAPI 后端 `/api/chat`、`/voice/*` 接口已够通用，眼镜端只需换采集端

**适配路径（远期）**：眼镜 SDK → 拍照 + 语音流 → 走现有 FastAPI 后端 → 精简文字回答或纯语音回传。真正的改造在「输入端换成眼镜摄像头/麦克风」和「输出端适配眼镜显示（字幕/纯语音）」，后端与 RAG/工具管道不变。

**当前依赖**：先把手机端「拍照定位」跑通（真机验证，对应 C3），再谈眼镜端硬件接入。
